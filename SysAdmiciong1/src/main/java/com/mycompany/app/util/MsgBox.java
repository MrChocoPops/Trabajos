...
public class MsgBox {
    ...
    public MsgBox(String msg, int tipoDialog, String iconPropio) {
        if (!iconPropio.trim().equals("")) {
            ...
            JOptionPane.showMessageDialog(p, msg, "AlertaMiloco", tipoDialog, icon);
        } else {
            JOptionPane.showMessageDialog(p, msg, "AlertaMiloco", tipoDialog);
        }
    }
...
// 0=ok, 2=cancel
        if (!iconPropio.trim().equals("")) {
            icon = new ImageIcon(obj.getFile(iconPropio));
            input = JOptionPane.showConfirmDialog(p, msg, title,
    ...
    public static void main(String[] args) {
        MsgBox msgBox = new MsgBox("Danger", JOptionPane.ERROR_MESSAGE,
